...
static void windows_transfer_callback(const struct windows_backend *backend,
	struct usbi_transfer *itransfer, uint32_t io_result, uint32_t io_size)
{
	int status, istatus;
	...
	switch (io_result) {
	case NO_ERROR:
		status = backend->copy_transfer_data(itransfer, io_size);
		break;
	...
	case ERROR_OPERATION_ABORTED:
		istatus = backend->copy_transfer_data(itransfer, io_size);
...
static int windows_init(struct libusb_context *ctx)
{
	...
	if (++init_count == 1) { // First init?
		...
		if (r == LIBUSB_SUCCESS) {
			...
			usbi_dbg("UsbDk backend unavailable");
...
static int windows_set_option(struct libusb_context *ctx, enum libusb_option option, va_list ap)
{
	...
	case LIBUSB_OPTION_USE_USBDK:
		...
		if (arg) {
			...
			usbi_dbg("switching context %p to use WinUSB backend", ctx);
			priv->backend = &winusb_backend;
		}
...
static int windows_handle_events(struct libusb_context *ctx, struct pollfd *fds, POLL_NFDS_TYPE nfds, int num_ready)
{
	...
	for (i = 0; i < nfds && num_ready > 0; i++) {
		...
		{
			pollable_fd = priv->backend->get_fd(itransfer);
		...
		if (found) {
			priv->backend->get_overlapped_result(itransfer, pollable_fd, &io_result, &io_size);
