...
// Default (unsupported) API prototypes
...
{
	union windows_device_priv *priv = (union windows_device_priv *)dev->os_priv;
	return &priv->winusb_priv;
}

static inline struct winusb_device_handle_priv *_device_handle_priv(
	struct libusb_device_handle *handle)
{
	union windows_device_handle_priv *priv =
		(union windows_device_handle_priv *)handle->os_priv;
	return &priv->winusb_priv;
}
...
static inline void winusb_device_priv_release(struct libusb_device *dev)
{
	...
	free(p->path);
	if ((dev->num_configurations > 0) && (p->config_descriptors != NULL)) {
		for (i = 0; i < dev->num_configurations; i++)
			free(p->config_descriptors[i]);
	}
...
/*
 * AdvAPI32, Cfgmgr32, OLE32 and SetupAPI DLL functions
 */
...
{
	struct winusb_device_priv *parent_priv, *priv = _device_priv(dev);
...
static int cache_config_descriptors(struct libusb_device *dev, HANDLE hub_handle, char *device_id)
{
	...
	priv->config_descriptors = calloc(dev->num_configurations, sizeof(PUSB_CONFIGURATION_DESCRIPTOR));
	...
	for (i = 0, r = LIBUSB_SUCCESS; ; i++) {
		...
		priv->config_descriptors[i] = malloc(cd_data->wTotalLength);
		if (priv->config_descriptors[i] == NULL)
			LOOP_BREAK(LIBUSB_ERROR_NO_MEM);
		memcpy(priv->config_descriptors[i], cd_data, cd_data->wTotalLength);
	}
	...
	if (r != LIBUSB_SUCCESS) {
		for (i = 0; i < dev->num_configurations; i++)
			free(priv->config_descriptors[i]);
	}
...
static int winusb_get_config_descriptor(struct libusb_device *dev, uint8_t config_index, unsigned char *buffer, size_t len)
{
	...
	if ((priv->config_descriptors == NULL) || (priv->config_descriptors[config_index] == NULL))
		return LIBUSB_ERROR_NOT_FOUND;

	config_header = priv->config_descriptors[config_index];

	size = MIN(config_header->wTotalLength, len);
	memcpy(buffer, priv->config_descriptors[config_index], size);
	return (int)size;
}
...
static struct winfd *winusb_get_fd(struct usbi_transfer *transfer)
{
	struct winusb_transfer_priv *transfer_priv = usbi_transfer_get_os_priv(transfer);
	return &transfer_priv->pollable_fd;
}

static void winusb_get_overlapped_result(struct usbi_transfer *transfer, struct winfd *pollable_fd, DWORD *io_result, DWORD *io_size)
{
